...
from llmcode import prompts, utils
...
class GitRepo:
    ...
    llmcode_ignore_spec = None
    llmcode_ignore_ts = 0
    llmcode_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if llmcode_ignore_file:
            self.llmcode_ignore_file = Path(llmcode_ignore_file)

    def commit(
        self, fnames=None, context=None, message=None, llmcode_edits=False, coder=None
    ):
        """
        ...
        Llmcode generated the changes (`llmcode_edits`).
        ...
        - llmcode_edits=True: Changes were generated by Llmcode (LLM).
        - llmcode_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        2. When llmcode_edits = False (User Changes):
           - --attribute-co-authored-by is IGNORED (trailer never added).
           - Author name is NEVER modified (--attribute-author ignored).
           - Committer name IS modified by default (implicit True, as Llmcode runs `git commit`).
           - EXCEPTION: If --attribute-committer is EXPLICITLY False, the name is NOT modified.

        Resulting Scenarios:
        - Standard AI edit (defaults): Co-authored-by=False -> Author=You(llmcode),
          Committer=You(llmcode)
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(llmcode), Committer=You, Trailer added
        - User commit (defaults): llmcode_edits=False -> Author=You, Committer=You(llmcode)
        - User commit with explicit no-committer: llmcode_edits=False,
        ...
        prefix_commit_message = llmcode_edits and (
        ...
        if llmcode_edits and attribute_co_authored_by:
        ...
        # Committer modification applies regardless of llmcode_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an llmcode edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (llmcode_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "llmcode: " + commit_message
        ...
        committer_name = f"{original_user_name} (llmcode)"
    ...
    def refresh_llmcode_ignore(self):
        if not self.llmcode_ignore_file:
            return

        current_time = time.time()
        if current_time - self.llmcode_ignore_last_check < 1:
            return

        self.llmcode_ignore_last_check = current_time

        if not self.llmcode_ignore_file.is_file():
            return

        mtime = self.llmcode_ignore_file.stat().st_mtime
        if mtime != self.llmcode_ignore_ts:
            self.llmcode_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.llmcode_ignore_file.read_text().splitlines()
            self.llmcode_ignore_spec = pathspec.PathSpec.from_lines(

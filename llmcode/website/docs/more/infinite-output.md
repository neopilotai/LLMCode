...
parent: More info
nav_order: 480
description: Llmcode can handle "infinite output" from models that support prefill.
---
...
Llmcode is able to work around this limit with models that support
"prefilling" the assistant response.
When you use llmcode with a model that supports prefill, you will see
...
When llmcode is collecting code edits from a model and
it hits the output token limit,
llmcode simply initiates another LLM request with the partial
response prefilled.
...
Llmcode supports "infinite output" for models that support "prefill",
such as:

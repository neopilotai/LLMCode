...
In most cases, you can safely ignore llmcode's warning about unknown context
...
configure llmcode with the proper token limits
for unusual models.

But, you can register context window limits and costs for models that aren't known
to llmcode. Create a `.llmcode.model.metadata.json` file in one of these locations:

- Your home directory.
- The root if your git repo.
- The current directory where you launch llmcode.
...
in the `.llmcode.model.metadata.json` file.
...
Llmcode has a number of settings that control how it works with
different models.
...
a model that llmcode doesn't know about.


### Configuration file locations

You can override or add settings for any model by creating a `.llmcode.model.settings.yml` file in one of these locations:
...
You can use the special model name `llmcode/extra_params` to define 
...
`llmcode/extra_params` settings taking precedence for any direct conflicts.
...
[models.py](https://github.com/khulnasoft-lab/llmcode/blob/main/llmcode/models.py)
...
<!--[[[cog
from llmcode.models import get_model_settings_as_yaml
cog.out("```yaml\n")

...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var blameData = {
        ...
        datasets: [{
            ...
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.llmcode_percentage }}, lines: {{ row.llmcode_total }}, end_date: '{{ row.end_date }}' },{% endfor %}],
    ...
    var linesData = {
        ...
        datasets: [{
            label: 'Llmcode',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.llmcode_total }} },{% endfor %}],
            backgroundColor: 'rgba(54, 162, 235, 0.8)',
            borderColor: 'rgba(54, 162, 235, 1)',
            borderWidth: 1
        },
        {
            label: 'Human',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.total_lines | minus: row.llmcode_total }} },{% endfor %}],

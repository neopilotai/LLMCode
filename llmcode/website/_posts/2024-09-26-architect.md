...
Llmcode now has experimental support for using two models to complete each coding task:
...
This approach has recently become attractive for llmcode due to 
...
Normally llmcode asks the model to solve a coding problem in one prompt,
asking the LLM to explain the solution and return 
a well formatted series of file edits.
All of [llmcode's editing formats](/docs/more/edit-formats.html)
require the LLM to return source code edits in a specific text
format, so that llmcode can process the edits and apply them to the local source files.
...
The development version of llmcode 
has built in defaults to support Architect/Editor coding with
o1-preview, o1-mini, GPT-4o and Claude 3.5 Sonnet.
Run llmcode with `--architect` or get started quickly like this:

```
pip install -U llmcode-chat
...
Llmcode has a number of "chat modes", and "architect" is available as a new chat mode.
The `--architect` switch is a shortcut for `--chat-mode architect`.
For more details, see documentation on 
[llmcode's chat modes](/docs/usage/modes.html).

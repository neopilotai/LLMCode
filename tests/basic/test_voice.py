...
from llmcode.voice import SoundDeviceError, Voice
...
@pytest.fixture
def mock_soundfile():
    with patch("llmcode.voice.sf") as mock_sf:
        yield mock_sf
...
def test_voice_init_invalid_format():
    with patch("llmcode.voice.sf", MagicMock()):  # Need to mock sf to avoid SoundDeviceError
        with pytest.raises(ValueError) as exc:

...
class TestRepo(unittest.TestCase):
    ...
    def test_commit_with_custom_committer_name(self, mock_send):
        ...
        with GitTemporaryDirectory():
            ...
            commit_result = git_repo_explicit_false.commit(
                fnames=[str(fname)], llmcode_edits=True
            )
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
    ...
    def test_commit_ai_edits_no_coauthor_explicit_false(self):
        ...
        with GitTemporaryDirectory():
            ...
            self.assertEqual(commit.author.name, "Test User")  # Explicit False
            self.assertEqual(
                commit.committer.name, "Test User (llmcode)"
            )  # Default True
    ...
    def test_get_tracked_files_with_llmcodeignore(self):
        with GitTemporaryDirectory():
            # new repo
            raw_repo = git.Repo()
            ...
            llmcodeignore = Path(".llmcodeignore")
            git_repo = GitRepo(InputOutput(), None, None, str(llmcodeignore))
            ...
            llmcodeignore.write_text("new.txt\n")
            time.sleep(2)
            ...
            #
            # llmcodeignore.write_text("new2.txt\n")

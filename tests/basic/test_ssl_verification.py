...
class TestSSLVerification(TestCase):
    ...
    @patch("llmcode.io.InputOutput.offer_url")
    @patch("llmcode.models.ModelInfoManager.set_verify_ssl")
    @patch("llmcode.llm.litellm._load_litellm")
    @patch("httpx.Client")
    @patch("httpx.AsyncClient")
    @patch("llmcode.models.fuzzy_match_models", return_value=[])
    def test_no_verify_ssl_flag_sets_model_info_manager(
        ...
        with patch("llmcode.models.Model") as mock_model:
            ...
            with patch("llmcode.llm.litellm._lazy_module", mock_module):
    ...
    @patch("llmcode.io.InputOutput.offer_url")
    @patch("llmcode.models.model_info_manager.set_verify_ssl")
    def test_default_ssl_verification(self, mock_set_verify_ssl, mock_offer_url):
        # Prevent actual URL opening
        mock_offer_url.return_value = False
        # Run main without --no-verify-ssl flag
        with patch("llmcode.main.InputOutput"):
            with patch("llmcode.coders.Coder.create"):

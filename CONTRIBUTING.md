...
Contributions of
[LLM benchmark results](https://llmcode.khulnasoft.com/docs/leaderboards/)
are welcome!
See the
...
[Individual Contributor License Agreement](https://llmcode.khulnasoft.com/docs/legal/contributor-agreement.html).
...
Note that these dependency files are generated by `./scripts/dev/generate_requirements.sh` and then committed. See [Managing Dependencies](#managing-dependencies).
...
```
docker build -t llmcode -f docker/app.Dockerfile .
```
...
The built documentation will be available in the `docs/site/_site` directory.

## Coding Standards

### Python Compatibility

Llmcode supports Python versions 3.9, 3.10, 3.11, and 3.12. When contributing code, ensure compatibility with these supported Python versions.
...
These workflows are triggered on push and pull request events to the `main` branch, ignoring changes to the `docs/site/**` and `README.md` files.
...
If you need to mock or create test data, consider adding it to the test files or creating separate fixtures or utility functions within the `llmcode/tests` directory.
...
```
./scripts/dev/generate_requirements.sh --upgrade
```

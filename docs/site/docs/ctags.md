...
Llmcode no longer uses ctags to build a repo map.
Please see the newer article about
[using tree-sitter to build a better repo map](https://llmcode.khulnasoft.com/docs/repomap.html).
...
To address these issues, `llmcode` now
...
[chat transcript](https://llmcode.khulnasoft.com/examples/add-test.html)
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [llmcode](https://llmcode.khulnasoft.com/docs/install.html).
  - Install universal ctags.
  - Run `llmcode` inside your repo, and it should say "Repo-map: universal-ctags using 1024 tokens".
...
The latest version of `llmcode` sends a **repo map** to GPT along with
...
and
[io.py](https://github.com/KhulnaSoft/llmcode/blob/main/llmcode/core/io.py)
:
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `llmcode` will automatically add them to the chat context (with user approval).
...
Sending an appropriate subset of the repo map would help `llmcode` work

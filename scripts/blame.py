...
def blame(start_tag, end_tag=None):
    ...
    files = [
        ...
        or (f.startswith("llmcode/resources/") and f.endswith(".yml"))
        ...
        results.append(
            {
                ...
                "llmcode_total": llmcode_total,
                "llmcode_percentage": round(llmcode_percentage, 2),
            }
        )
    return results
...
def main():
    parser = argparse.ArgumentParser(description="Get llmcode/non-llmcode blame stats")
    ...
    if not args.all_since:
        print(f"- Llmcode wrote {round(llmcode_percentage)}% of the code in this release.")

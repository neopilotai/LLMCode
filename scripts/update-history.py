...
def main():
    ...
    next_version_idx = history_content.find("\n### Llmcode v", version_idx + len(version_header))
    ...
    llmcode_line = blame_result.stdout.strip().split("\n")[-1]  # Get last line with percentage

    # Construct and run the llmcode command
    message = history_prompt.format(llmcode_line=llmcode_line)
    ...
    # Run update-docs.sh after llmcode
